...
class WindowAddTransaction(customtkinter.CTk):
    ...
    def window_widgets(self):
        ...
        # create title
        title = customtkinter.CTkLabel(self.transaction_window, text="Transaction Entry", corner_radius=10)
        ...
        name_of_security_entry = customtkinter.CTkComboBox(self.transaction_window, values=self.master.db.get_table_value("security_name"))
        name_of_security_entry.set(value="")
        name_of_security_entry.place(relx=0.16, rely=0.125, relwidth=0.82, relheight=0.05)

        ticker_entry = customtkinter.CTkComboBox(self.transaction_window, values=self.master.db.get_table_value("security_ticker"))
        ticker_entry.set(value="")
        ticker_entry.place(relx=0.16, rely=0.225, relwidth=0.82, relheight=0.05)

        institution_name_entry = customtkinter.CTkComboBox(self.transaction_window, values=self.master.db.get_table_value("institution"))
        ...
        time_of_transaction_entry = customtkinter.CTkEntry(self.transaction_window, placeholder_text=tot_text)
        time_of_transaction_entry.place(relx=0.16, rely=0.425, relwidth=0.82, relheight=0.05)

        amount_of_shares_entry = customtkinter.CTkEntry(self.transaction_window, placeholder_text="Input only a number.")
        amount_of_shares_entry.place(relx=0.16, rely=0.525, relwidth=0.82, relheight=0.05)

        price_usd_entry = customtkinter.CTkEntry(self.transaction_window, placeholder_text="Input only a number. Do not input the currency.")
        price_usd_entry.place(relx=0.16, rely=0.625, relwidth=0.82, relheight=0.05)

        transaction_type_entry = customtkinter.CTkOptionMenu(self.transaction_window, values =["Acquire", "Dispose"], fg_color =("#F9F9FA", "#343638"),
        ...
        # entry button
        entry_button = customtkinter.CTkButton(self.transaction_window, text="Enter", command=lambda: self.master.gb.insert_transaction_into_database(entry_dic))
        entry_button.place(relx=0, rely=0.825, relwidth=1, relheight=0.05)

        # return to main menu and exit button
        return_main_button = customtkinter.CTkButton(self.transaction_window, text="Close Window", command=self.transaction_window.destroy)
        exit_button=customtkinter.CTkButton(self.transaction_window, text="Exit Program", command=self.master.on_closing)
